...
defineExperiment<-function()
{
  if(file.exists("R/paramsets.Rdata")) {
    load(paste(.libPaths(), "RAMClustR/R/paramsets.Rdata", sep=""))} else {load(paste(.libPaths(), "RAMClustR/R/defparamsets.Rdata", sep=""))
    }
  ...
  if(!(instrument["saveAs",1] %in% platforms) & instrument["saveAs",1] != "") 
                                  ...
                                  save(paramsets, file=paste(libPaths(), "RAMClustR/R/paramsets.Rdata", sep=""))
                                  }
