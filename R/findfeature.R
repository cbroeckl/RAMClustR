...
#' @param rt numeric: optional rt value to search for (generally in seconds, though use whatever units your data is in)
#' @param rttol numeric: absolute retention time tolerance around rt.
#' @param iso.rttol numeric: when examining isotope patterns, feature retention time tolerance around features matching mz +- mztol
...
findfeature <- function (ramclustObj = NULL, 
                      mz = NULL, 
                      mztol = 0.02, 
                      rt = NULL,
                      rttol = 2,
                      ...
                      m.check = TRUE) 
{
  ...
  if(is.numeric(rt)) {
    tar <- which((abs(ramclustObj$fmz - mz) <= mztol) & (abs(ramclustObj$frt - rt) <= rttol))
    } else {
        ...
        for (j in 1:zmax) {
          check1 <- which((abs(ramclustObj$fmz - mz + 
                                 1.007276) <= mztol) & (abs(ramclustObj$frt - 
                                                              ramclustObj$frt[tar[i]]) <= iso.rttol))
