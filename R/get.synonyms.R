...
#' @param get.db logical - should the HMDB, LipidMaps, and Pubchem names be retreived when available?
...
#' @param update.names logical - should the ramclustObj$ann slot (annotation) be updated based on the above? 
...
get.synonyms <- function(ramclustObj = RC,
  ...
  if(get.db) {
    ...
    for(x in 1:length(ramclustObj$cmpd)) {
      ...
      out<- tryCatch(suppressWarnings(read_xml(link)), error = function(x) {return(NA)})
      if(!is.na(out)) {
        baz <- xml_find_all(out, ".//name")
        hmdb.name[x] <- xml_text(baz, "name")[1]
      } else {hmdb.url[x] <- NA}
    }
    ...
    for(x in 1:length(ramclustObj$cmpd)) {
      ...
      if(out[1] != "stuff.and.nonsense") {
        out <- fromJSON(out)$InformationList$Information[,"Title"]
  ...
  if(lipid.short.hand){
    ...
    for(x in 1:length(lmname)) {
      ...
      tars <- synonyms[[x]][str_detect(synonyms[[x]], "\\([0-9]{1,2}\\:[0-9]{1,2}\\)")]
