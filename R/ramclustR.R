...
ramclustR <- function(xcmsObj = NULL,
  ...
  if (!is.null(ms)) {
    ramclustObj <- rc.get.csv.data(csv = ms, st = st)
    st <- ramclustObj$st
  ...
  if (!is.null(xcmsObj)) {
    ...
    if (newXCMS) {
      sampnames <- xcmsObj@phenoData[[1]]
    }
    if (!newXCMS) {
      data12 <- xcms::groupval(xcmsObj, value = "into")
    }
    if (newXCMS) {
      data12 <- xcms::featureValues(xcmsObj, value = "into")
    }
    ...
    if (taglocation == "filepaths" & !is.null(MStag)) {
      ...
      if (newXCMS) {
        mzs <- round(xcmsObj@msFeatureData$featureDefinitions$mzmed, digits = 4)
      }
    } else {
      data1 <- t(data12)
  ...
  if (replace.zeros == TRUE) {
    ...
    rpl2 <- which(data2 == 0)
    if (length(rpl1) > 0) {
      data1[rpl1] <- abs(jitter(rep(min(data1, na.rm = TRUE), length(rpl1)), amount = min(data1 / 100, na.rm = TRUE)))
    }
    if (length(rpl2) > 0) {
      data2[rpl2] <- abs(jitter(rep(min(data2, na.rm = TRUE), length(rpl2)), amount = min(data2 / 100, na.rm = TRUE)))
    }
  }
  ...
  if (collapse == "TRUE") {
    ...
    if (!is.null(ms) & normalize == "quantile") {
      dimnames(ramclustObj$SpecAbund)[[1]] <- tmpnames1[[1]]
    }
    ...
    if (usePheno & !is.null(xcmsObj)) {
      if (!newXCMS) {
        dimnames(ramclustObj$SpecAbund)[[1]] <- as.vector(xcmsObj@phenoData[, 1])[msfiles]
      }
      if (newXCMS) {
        dimnames(ramclustObj$SpecAbund)[[1]] <- as.vector(xcmsObj@phenoData[[1]])[msfiles]
      }
  ...
  if (mspout == TRUE) {
    ...
    for (m in 1:as.numeric(mslev)) {
      for (j in 1:max(ramclustObj$featclus)) {
        ...
        if (m == 1) {
          wts <- rowSums(ramclustObj$MSdata[, sl, drop = FALSE])
          for (k in 1:length(sl)) {
            wm[k] <- weighted.mean(ramclustObj$MSdata[, sl[k]], wts)
          }
        }
        if (m == 2) {
          wts <- rowSums(ramclustObj$MSMSdata[, sl, drop = FALSE])
          for (k in 1:length(sl)) {
