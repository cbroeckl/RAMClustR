...
rc.qc <- function(ramclustObj = NULL,
  ...
  if (!is.null(ramclustObj$clrt) && do.plot) {
    ...
    title(
      main = "histogram of pearson's r for each cluster to its adjacent cluster (by time)", cex.main = 0.8,
      sub = paste(
        "skew =", round(e1071::skewness(d), digits = 3), " :values near zero are better", "\n",
        "WARNING:metabolic relationships will confound interpretation of this plot"
      ), cex.sub = 0.6
    )
    ...
    gplots::heatmap.2(c^2,
      trace = "none", dendrogram = "none", Rowv = FALSE, Colv = FALSE, main = "pearsons r^2, clusters sorted by rt", cex.main = 0.5,
    ...
    if (do.plot) {
      ...
      for (i in 1:(ncol(sc) - 1)) {
        plot(sc[, i], sc[, i + 1],
          ...
          xlab = paste(
            "PC", i, "::  r^2 =", round(PCA@R2[i], digits = 2), "::  QC(rel sd) = ",
            round(sd(sc[qc, i]) / sd(sc[, i]), digits = 2)
          ),
          ylab = paste(
            "PC", i + 1, "::  r^2 =", round(PCA@R2[i + 1], digits = 2), "::  QC(rel sd) = ",
  ...
  ramclustObj$history$qc.summary <- paste(
    ...
    if (!is.null(ramclustObj$cmpd.use)) {
      " Only compounds which passed the CV filter are reported."
    }
  )
